...
/// TODO: Assert is target is x86 before using x86 instrinsics
/// TODO: Target ARM CPUs with NEON intrinsics

/// @brief SIMD SSE 1D boxcar filter
///
/// Applies a boxcar filter to the data array
/// In y and z axis of the image cube data access is not coalesced
/// so data is copied into a temporary array. By fetching and processing
/// with SIMD intrinsics each fetch can process multiple data points simentaneously
...
void filter_boxcar_1d_flt_avx(float *data, __m256 *data_copy, const size_t size, const size_t stride, const size_t filter_radius) {
	...
	for (int col = size - 1; col--;) {
		...
		// GCC would optimize this with an FMA if it's available
		current_pt = _mm256_mul_ps(current_pt, inv_filter_size_v);
		current_pt = _mm256_add_ps(current_pt, next_pt);
