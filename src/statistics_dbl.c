...
//                                                                      //
// SoFiA 2.5.0 (statistics_dbl.c) - Source Finding Application          //
...
/// @file   statistics_dbl.c
/// @author Tobias Westmeier
/// @date   26/11/2021
/// @brief  Function templates for basic statistical analysis of data of type `double`.
...
void max_min_dbl(const double *data, const size_t size, double *value_max, double *value_min)
{
	...
	*value_max = *ptr;
	...
	for(const double *ptr2 = ptr; ptr2 --> data;)
	{
...
/// @brief Sum
///
/// Wrapper function for summation_dbl() that will return the
...
/// @brief Median (safe)
///
/// Calculates the median of the input data array. Unlike median_dbl(),
...
double kin_maj_axis_dbl(const double *centroidX, const double *centroidY, const double *sum, const size_t size, const size_t first, const size_t last)
{
	...
	const double slope = (sumYY - sumXX + sqrt((sumYY - sumXX) * (sumYY - sumXX) + 4.0 * sumXY * sumXY)) / (2.0 * sumXY);
	//const double inter = sumY - slope * sumX;
	
	// Calculate position angle of kinematic major axis:
	double pa = atan(slope);
	...
	const double difference = fabs(atan2(sin(fullAngle) * cos(pa) - cos(fullAngle) * sin(pa), cos(fullAngle) * cos(pa) + sin(fullAngle) * sin(pa)));
