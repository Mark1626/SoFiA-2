...
//                                                                      //
// SoFiA 2.5.0 (Array_dbl.c) - Source Finding Application               //
...
/// @date   25/11/2021
/// @brief  Container class template for storing a dynamic array of values of type `double`.
...
///
/// @return Pointer to newly created Array_dbl object.
...
PUBLIC Array_dbl *Array_dbl_new_str(const char *string)
{
	...
	if(!strlen(string)) return Array_dbl_new(0);
...
PUBLIC Array_dbl *Array_dbl_cat(Array_dbl *self, const Array_dbl *source)
{
	...
	self->values = (double *)memory_realloc(self->values, self->size + source->size, sizeof(double));
