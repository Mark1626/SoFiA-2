...
/// @date   10/12/2021
/// @brief  Function templates for basic statistical analysis of data of type
/// `float` (header).
...
// -------------------------- //
#define BOXCAR_MIN_ITER                                                        \
  3 ///< Minimum number of iterations required for boxcar approximation of
    ///< Gaussian smoothing kernel.
#define BOXCAR_MAX_ITER                                                        \
  6 ///< Maximum number of iterations allowed for boxcar approximation of
    ///< Gaussian smoothing kernel.
...
// Maximum and minimum
void max_min_flt(const float *data, const size_t size, float *value_max,
                 float *value_min);
...
// N-th moment
double moment_flt(const float *data, const size_t size, unsigned int order,
                  const double value);
void moments_flt(const float *data, const size_t size, const double value,
                 double *m2, double *m3, double *m4);

// Standard deviation
double std_dev_flt(const float *data, const size_t size);
double std_dev_val_flt(const float *data, const size_t size, const double value,
                       const size_t cadence, const int range);
...
float mad_val_flt(const float *data, const size_t size, const float value,
                  const size_t cadence, const int range);
...
float robust_noise_in_region_flt(const float *data, const size_t nx,
                                 const size_t ny, const size_t x1,
                                 const size_t x2, const size_t y1,
                                 const size_t y2, const size_t z1,
                                 const size_t z2);

// Gaussian fit to histogram
size_t *create_histogram_flt(const float *data, const size_t size,
                             const size_t n_bins, const float data_min,
                             const float data_max, const size_t cadence);
float gaufit_flt(const float *data, const size_t size, const size_t cadence,
                 const int range);

// Skewness and kurtosis
void skew_kurt_flt(const float *data, const size_t size, double *skew,
                   double *kurt);
...
#ifdef __AVX2__
void filter_boxcar_1d_flt_sse(float *data, const size_t size,
                              const size_t stride, const size_t filter_radius);
void filter_boxcar_1d_flt_avx(float *data, __m256 *data_copy, const size_t size,
                              const size_t stride, const size_t filter_radius);
#endif
...
void filter_gauss_2d_flt(float *data, float *data_copy, float *data_row,
                         ...
                         const size_t filter_radius);
#ifdef __AVX2__
void filter_gauss_2d_flt_avx(float *data, __m256 *data_copy, float *data_row,
                             float *data_col, const size_t size_x,
                             const size_t size_y, const size_t n_iter,
                             const size_t filter_radius);
#endif
...
void moment_ellipse_fit_flt(const float *moment_map, const size_t *count_map,
                            const size_t size_x, const size_t size_y,
                            const float centroid_x, const float centroid_y,
                            const float rms, float *ell_maj, float *ell_min,
                            float *ell_pa, float *ell3s_maj, float *ell3s_min,
                            float *ell3s_pa);
void spectral_line_width_flt(const float *spectrum, const size_t size,
                             float *w20, float *w50);
double wm50_line_width_flt(const float *spectrum, const size_t size);
float kin_maj_axis_flt(const float *centroidX, const float *centroidY,
                       const float *sum, const size_t size, const size_t first,
                       const size_t last);
...
void optimal_filter_size_flt(const double sigma, size_t *filter_radius,
                             size_t *n_iter);
